...
const SignInForm = () => {
  ...
  const [googlePending, startGoogleTransaction] = useTransition()
  ...
  async function signInWithGoogle() {
    startGoogleTransaction(async () => {
      await authClient.signIn.social({
        ...
        fetchOptions: {
          onError: () => {
            toast.error("Login with Google failed. Please try again or use another login method.")
          }
        }
      })
    })
  ...
  const shouldDisable = emailPending || githubPending || googlePending
  return (
    <Card className="bg-transparent border-none">
      ...
      <CardContent className="flex flex-col gap-4">
        <Button disabled={shouldDisable} variant="secondary" className="w-full" onClick={signInWithGithub}>
          {
            githubPending ?
              <Spinner /> :
              <>
        ...
        <Button disabled={shouldDisable} variant="secondary" className="w-full" onClick={signInWithGoogle}>
          {
            googlePending ?
              <Spinner /> :
              <>
